...
export default function App() {
    ...
    try {
      ...
      setOrders(prev => {
        ...
        const mergedImported = importedOrders.map(imported => {
          ...
          const keepLocalEta = keepLocalArrived && !!normalizeEta(existing.eta);

          return {
            ...imported,
            status: keepLocalArrived ? 'arrived' : imported.status,
            arrived: keepLocalArrived ? true : imported.arrived,
  ...
  }, [plannedActiveOrders, plannerLineFilter, plannerSearch]);
  ...
  useEffect(() => {
    if (!plannerDrivers.length) {
      setSelectedDriverName('');
      return;
    }
    if (selectedDriverName && !plannerDrivers.some(driver => driver.name === selectedDriverName)) {
      setSelectedDriverName(plannerDrivers[0].name);
    }
  }, [plannerDrivers, selectedDriverName]);
  ...
  const handleAssignDriverToOrder = (orderId: number, driverName: string) => {
  ...
  const handleClearDriverFromOrder = (orderId: number) => {
    setOrders(prev => {
      const target = prev.find(order => order.id === orderId);
      if (!target) return prev;
      const rit = (target.rit || '').trim();
                              ...
                              truckOrders
                                ...
                                .map(o => (
                                  <tr key={o.id}>
                                    ...
                                    <td className="font-semibold">{o.vol} m3</td>
                                    <td>
                      ...
                      <div className="grid grid-cols-1 xl:grid-cols-[340px_1fr] gap-4">
                        <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-4">
                          <div className="mb-4">
                            ...
                            <div className="text-sm text-gray-500">Lijst uit de huidige orders. Klik om de dagvrachten te zien of sleep naar een vracht.</div>
                          </div>

                          <div className="space-y-2 max-h-[70vh] overflow-y-auto pr-1">
                            {plannerDrivers.length > 0 ? plannerDrivers.map(driver => (
                              <button
                                key={driver.name}
                                draggable
                              ...
                              <div className="rounded-xl border border-dashed border-gray-200 p-6 text-center text-sm text-gray-400">
                                Nog geen chauffeurs gevonden in de orders
                              </div>
                            )}
                          </div>
                        </div>
                              ...
                              {selectedDriverOrders.length > 0 ? (
                                <div className="mt-4 space-y-2">
                                  {selectedDriverOrders.map(o => (
                                    <div key={`selected-${o.id}`} className="rounded-xl border border-blue-200 bg-white px-3 py-2 text-sm text-blue-900">
                                      <div className="font-semibold">{o.customer}</div>
                                      <div className="text-xs text-blue-700">
                                        Order {o.num} - Rit {o.rit || '--'} - {ev(o).toFixed(1)} m3 - Lijn {o.line} - {o.eta || '--'}
                                      </div>
                                    </div>
                                  ))}
                                </div>
                              ) : (
                                <div className="mt-3 text-sm text-blue-700">Nog geen gekoppelde vrachten.</div>
                              )}
                            </div>
                          )}
                                    ...
                                    <div className="flex items-center justify-between gap-6">
                                      <div className="min-w-0 flex-1">
                                        ...
                                        <div className="text-sm text-gray-400 mt-1">
                                          {o.num} - Rit {o.rit || '--'} - {o.recipe} - {ev(o).toFixed(1)} m3
                                        </div>
