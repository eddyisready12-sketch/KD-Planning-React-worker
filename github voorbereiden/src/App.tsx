...
import { fetchBunkerStateFromSupabase, fetchDriverListFromSupabase, fetchIssuesFromSupabase, fetchOrdersFromSupabase, fetchPlannedOrderIdsFromSupabase, isSupabaseConfigured, resolveIssueInSupabase, writeBunkersToSupabase, writeDriverListToSupabase, writeIssueToSupabase, writeOrdersToSupabase, writePlannedOrderIdsToSupabase } from './services/supabaseService';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'shared_order_components' },
        refreshOrdersFromSupabase
      )
      .subscribe();
  ...
  const handleRecalculate = useCallback(() => {
    ...
    setTimeout(() => {
      try {
        ...
        if (isSupabaseConfigured()) {
          void writePlannedOrderIdsToSupabase(next);
        }
      } catch (error) {
  ...
  const nextPlannedOrder = useMemo(
    () => plannedEntries[0]?.order || null,
    [plannedEntries]
  );
  ...
  const nextNeedsProlineCleaning = useMemo(
    () => hasProlineCleaningTrigger(nextPlannedOrder),
    [nextPlannedOrder]
  );
  ...
  const getOperatorOrderState = (
    ...
    const etaMinutes = etaToMins(normalizeEta(order.eta));
    ...
    const needsCleaning = index === 0 && nextNeedsProlineCleaning;
    const bunkerPrepReason = getOrderBunkerPrepReason(order);
    const needsPrep = needsRecipeBunkerPrep || (index === 0 && nextOrderBunkerSwitches.length > 0);
    const earlierOrders = plannedEntries.slice(0, index);
    const blockedByEarlierSpecialStep = index > 0 && earlierOrders.some((earlierEntry, earlierIndex) => {
      const earlierOrder = earlierEntry.order;
      const earlierIssue = storingen[earlierOrder.line];
      const earlierEtaMinutes = etaToMins(normalizeEta(earlierOrder.eta));
      const earlierWaitsForBulk = earlierOrder.pkg === 'bulk' && earlierEtaMinutes !== null && earlierEtaMinutes > nowMinutes;
      const earlierNeedsRecipeBunkerPrep = !isOrderDirectlyRunnable(earlierOrder);
      const earlierNeedsPrep = earlierIndex === 0 && nextOrderBunkerSwitches.length > 0;
      return earlierNeedsRecipeBunkerPrep || earlierNeedsPrep || earlierWaitsForBulk || !!earlierIssue?.actief;
    });
    const blockedByEarlierOrder = index > 0;
    ...
    if (blockedByEarlierSpecialStep) {
      return { label: 'Wachten', cls: 'bg-blue-100 text-blue-700', reason: 'Wacht op vorige blokkade', key: 'wait' };
    }
    if (blockedByEarlierOrder) {
      return { label: 'Wachten', cls: 'bg-blue-100 text-blue-700', reason: 'Wacht op vorige order', key: 'wait' };
    }
    if (needsCleaning) {
  ...
  const getPlannerOrderState = (
    ...
    const earlierOrders = lineEntries.slice(0, index);
    const blockedByEarlierSpecialStep = index > 0 && earlierOrders.some((earlierEntry, earlierIndex) => {
      const earlierOrder = earlierEntry.order;
      const earlierIssue = storingen[earlierOrder.line];
      const earlierEtaMinutes = etaToMins(normalizeEta(earlierOrder.eta));
      const earlierWaitsForBulk = earlierOrder.pkg === 'bulk' && earlierEtaMinutes !== null && earlierEtaMinutes > nowMinutes;
      const earlierNeedsRecipeBunkerPrep = !isOrderDirectlyRunnable(earlierOrder);
      return earlierNeedsRecipeBunkerPrep || earlierWaitsForBulk || !!earlierIssue?.actief;
    });
    ...
    if (blockedByEarlierSpecialStep || index > 0) {
      return {
        label: 'Wachten',
        cls: 'bg-blue-100 text-blue-700',
        reason: blockedByEarlierSpecialStep ? 'Wacht op vorige blokkade' : 'Wacht op vorige order',
        key: 'wait'
      };
    }
    if (needsCleaning) {
  ...
  const nextOrderBunkerSwitches = useMemo(() => {
    const upcomingOrders = plannedEntries.slice(0, 2).map(entry => entry.order);
    const nextOrder = upcomingOrders[0];
    if (!nextOrder) return [];
    const { bulkComponents: nextOrderBulkComponents, assignments } = getOrderBunkerAssignments(nextOrder);
  ...
  }, [plannedEntries, bunkers, selectedLine]);

  const operatorExecutionCards = useMemo(() => {
    const nowMinutes = currentTime.getHours() * 60 + currentTime.getMinutes();
    return plannedEntries.slice(0, 3).map((entry, index) => {
      const order = entry.order;
      const operatorState = getOperatorOrderState(order, index, nowMinutes);
  ...
  }, [plannedEntries, currentTime, nextNeedsProlineCleaning, nextOrderBunkerSwitches]);
              ...
              <div className="max-w-6xl mx-auto">
                ...
                {nextNeedsProlineCleaning && (
                  <div className="bg-blue-50 border border-blue-200 text-blue-900 px-4 py-3 rounded-lg mb-4">
                    ...
                    <div className="text-sm">
                      Volgende order vraagt een Proline reinigingsactie: <span className="font-semibold">{nextPlannedOrder?.customer}</span>
                    </div>
                  </div>
                )}
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-2 mb-3.5">
                      {plannedEntries.length > 0 ? plannedEntries.map((entry, i) => {
                        ...
                        const nowMinutes = currentTime.getHours() * 60 + currentTime.getMinutes();
                        const operatorState = getOperatorOrderState(o, i, nowMinutes);
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button 
                      ...
                      disabled={dataSource.loading}
                    >
                      {dataSource.loading ? 'Bezig...' : 'Sync Sheet naar Supabase'}
                    </button>
                  </div>
                </div>
                            ...
                            <div key={lid} className="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                              ...
                              <div className="divide-y divide-gray-100">
                                {timeline.length > 0 ? timeline.map((entry, i) => {
                                  ...
                                  const plannerState = getPlannerOrderState(o, i, timeline, currentTime.getHours() * 60 + currentTime.getMinutes());
                          ...
                          <tbody>
                            {plannedActiveOrders
                              ...
                              .map(o => {
                                ...
                                const timelineIndex = timelineEntry ? lineEntries.findIndex(entry => entry.order.id === o.id) : -1;
                                const plannerState = timelineIndex >= 0
                                  ? getPlannerOrderState(o, timelineIndex, lineEntries, currentTime.getHours() * 60 + currentTime.getMinutes())
                                  : null;
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="pt-2 border-t border-gray-100 flex items-center justify-between">
                      <div className="text-xs text-gray-500 italic">
                        Laatste sync: {dataSource.lastSync ? new Date(dataSource.lastSync).toLocaleString('nl-NL') : 'Nooit'} - Auto-sync elke 60 sec
                      </div>
                      <div className="flex gap-2">
                        <button 
                          className="btn btn-p btn-sm" 
