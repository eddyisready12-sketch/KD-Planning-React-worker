...
function rowToOrder(row: SharedOrderRow, components: OrderComponent[]): Order | null {
  ...
  return {
    ...
    vol: isNaN(vol) ? 0 : vol,
    pkg,
    prio: ([1, 2, 3].includes(priority) ? priority : (pkg === 'bale' || pkg === 'bag' ? 1 : 2)) as 1 | 2 | 3,
    status,
...
export async function fetchOrdersFromSupabase(): Promise<Order[]> {
  ...
  let componentRows: SharedComponentRow[] = [];
  try {
    componentRows = await supabaseFetch<SharedComponentRow[]>(
      `shared_order_components?workspace=eq.${encodeURIComponent(SUPABASE_WORKSPACE)}&select=*`
    );
  } catch {
    componentRows = [];
  }

  const componentsByOrder = new Map<string, OrderComponent[]>();
  componentRows.forEach(row => {
    if (!row.order_num) return;
    const component: OrderComponent = {
      name: String(row.component_name || row.material_name || row.name || ''),
      code: String(row.component_code || row.material_code || row.code || ''),
      value: parseNumber(row.ratio ?? row.value),
      unit: String(row.unit || '')
    };
    if (!component.name && !component.code) return;
    const list = componentsByOrder.get(String(row.order_num)) || [];
    if (!list.some(existing =>
      ...
      list.push(component);
      componentsByOrder.set(String(row.order_num), list);
    }
  });

  return orders
    .map(row => rowToOrder(row, componentsByOrder.get(String(row.order_num)) || []))
...
export async function writeOrdersToSupabase(orders: Order[]): Promise<void> {
  ...
  await supabaseWrite('shared_orders?on_conflict=workspace,order_num', orderRows);

  const persistedOrders = await supabaseFetch<Array<{ id: string; order_num: string }>>(
    `shared_orders?workspace=eq.${encodeURIComponent(SUPABASE_WORKSPACE)}&select=id,order_num`
  );
  const orderIdByOrderNum = new Map(
    persistedOrders
      .filter(row => !!row.order_num)
      .map(row => [String(row.order_num), row.id])
  );
  ...
  orders.forEach(order => {
    (order.components || []).forEach(component => {
      ...
      componentMap.set(key, {
        order_id: orderIdByOrderNum.get(order.num) || null,
        workspace: SUPABASE_WORKSPACE,
        order_num: order.num,
  ...
  if (componentRows.length > 0) {
    await supabaseWrite('shared_order_components?on_conflict=workspace,order_num,component_code,component_name', componentRows);
  }
}
