...
export function ev(o: Order): number {
  return (o.pkg === 'bale' || o.pkg === 'packaged' || o.pkg === 'bag') ? o.vol * 1.03 : o.vol;
}
...
export function normalizePkg(value: string): 'bulk' | 'bag' | 'bale' | 'packaged' {
  const raw = String(value || '').trim().toUpperCase();
  if (raw === 'M3') return 'bulk';
  if (raw === 'BAL') return 'bale';
  if (raw === 'BAG') return 'bag';
  return 'bulk'; // Default to bulk as requested
}

export function etaToMins(eta: string): number | null {
  if (!eta || !eta.trim()) return null;
  const p = eta.split(':');
